...
After a couple of days I decided to check on the cluster again and I noticed that I couldnt access it again, so I repeated the same drill, and noticed that the memory usage was a bit too high, and only scaled down the control plane node down to 6GB of RAM.
